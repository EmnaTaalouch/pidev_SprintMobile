 ...
 * @author mariem
 */
public class StatTypeCompta extends BaseForm{
        Form current;

        public StatTypeCompta(Resources res) {
...
protected CategorySeries buildCategoryDataset(String title, double[] values) {
    ...
    */
             List<TypeComptabilite> lista = ServiceTypeComptabilite.getInstance().getAllTypeComptabilites();;
             ServiceComptabilite sc = new ServiceComptabilite();
             for (TypeComptabilite fi : lista) {
                 int tmp=0;
                    ArrayList <Comptabilite> comptabilites = new ArrayList();
                    ServiceComptabilite sa =new ServiceComptabilite();
                    comptabilites=sa.getByTypeComptabilites(fi.getId());

                 for (Comptabilite fii : comptabilites) {  
                     tmp++;
                      }
                 
            series.add(fi.getType() ,tmp);
             }
